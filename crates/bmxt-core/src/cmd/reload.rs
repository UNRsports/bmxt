...
/** EN: Bare `reload` → active tab. Numeric ids → pipe/programmatic targets only (no picker). */
pub fn run(args: &[String]) -> DispatchBundle {
    match parse_optional_tab_ids(args) {
        ...
        Err(()) => msgs(vec![
            msg_key("cmd.reload.error.badArgs"),
            msg_key("cmd.reload.usage"),
        ]),
    }
}
...
mod tests {
    ...
    fn parses_numeric_ids() {
        let bundle = run(&["reload".into(), "12".into(), "34".into()]);
        match bundle {
            DispatchBundle::Effects { effects } => match &effects[0] {
