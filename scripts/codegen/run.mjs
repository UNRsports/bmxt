...
function loadManifest() {
  ...
  if (!Array.isArray(m.uiActions)) {
    throw new Error("manifest must have uiActions[]")
  }
  validateCommandsSubcommands(m.commands)
  validateUiActions(m.uiActions)
  return m
}

function validateUiActions(uiActions) {
  const seen = new Set()
  for (const ua of uiActions) {
    if (!ua || typeof ua !== "object") {
      throw new Error("uiActions[] entry must be an object")
    }
    if (typeof ua.kind !== "string" || ua.kind.length === 0) {
      throw new Error("uiActions[] needs non-empty kind")
    }
    if (typeof ua.rustVariant !== "string" || ua.rustVariant.length === 0) {
      throw new Error(`uiActions kind=${ua.kind}: needs rustVariant`)
    }
    if (ua.shape !== "unit" && ua.shape !== "record") {
      throw new Error(`uiActions kind=${ua.kind}: shape must be unit|record`)
    }
    if (seen.has(ua.kind)) {
      throw new Error(`uiActions duplicate kind: ${ua.kind}`)
    }
    seen.add(ua.kind)
    if (ua.shape === "record") {
      if (!ua.fields || typeof ua.fields !== "object") {
        throw new Error(`uiActions kind=${ua.kind}: record needs fields`)
      }
      for (const [k, t] of Object.entries(ua.fields)) {
        if (t !== "string" && t !== "i32" && t !== "vec_i32") {
          throw new Error(`uiActions kind=${ua.kind} field ${k}: unsupported type ${t}`)
        }
      }
    }
  }
...
function uiActionFields(ua) {
  if (ua.shape === "unit") {
    return null
  }
  return ua.fields ?? null
}

function emitTsEffectTypes(effects, uiActions) {
  const members = []
  ...
  const uiMembers = uiActions.map((ua) => {
    const parts = [`kind: ${JSON.stringify(ua.kind)}`]
    const fields = uiActionFields(ua)
    if (fields) {
      for (const [k, t] of Object.entries(fields).sort()) {
        const tsType = t === "i32" ? "number" : t === "vec_i32" ? "number[]" : "string"
        parts.push(`${k}: ${tsType}`)
      }
    }
    return `  | { ${parts.join("; ")} }`
...
function emitTsUiActionTypes(uiActions) {
  const uiMembers = uiActions.map((ua) => {
    const parts = [`kind: ${JSON.stringify(ua.kind)}`]
    const fields = uiActionFields(ua)
    if (fields) {
...
function emitRustUiAction(uiActions) {
  const variants = []
  for (const ua of [...uiActions].sort((a, b) => a.kind.localeCompare(b.kind))) {
    if (ua.shape === "unit") {
      variants.push(`    #[serde(rename = ${JSON.stringify(ua.kind)})]\n    ${ua.rustVariant},`)
      continue
    }
    const fieldLines = []
    for (const [key, t] of Object.entries(ua.fields).sort()) {
      const ident = rustFieldIdent(key)
      fieldLines.push(`${rustSerdeRename(key)}    ${ident}: ${rustFieldType(t)},`)
    }
    variants.push(
      `    #[serde(rename = ${JSON.stringify(ua.kind)})]\n    ${ua.rustVariant} {\n${fieldLines.join("\n")}\n    },`
    )
  }
...
function main() {
  ...
  writeFileSync(
    ...
    emitTsEffectTypes(m.effects, m.uiActions),
    "utf8"
  )
  ...
  writeFileSync(
    join(rustGeneratedDir, "ui_action.rs"),
    emitRustUiAction(m.uiActions),
    "utf8"
  )
  writeFileSync(
  ...
  writeFileSync(
    ...
    emitTsUiActionTypes(m.uiActions),
    "utf8"
  )

  console.log(
    "codegen ok: registry/table.gen.ts, effect-types, ui-action-types, apply-dispatch.gen, completion-fallback, command-subcommands.gen.ts, crates/bmxt-core/src/generated/{chrome_effect,ui_action,registry_table}.rs"
  )
}
