...
**Tab chips (`tab:`):** Type **`tab:`** to open open-tab candidates (favicon + title). **`tab:{needle}`** filters by title; **`tab::{needle}`** filters by URL (URL shown on rows). Pick inserts a **`#t:<id>`** chip and re-appends **`tab:`** so you can select more. Esc or ↑ at the top of the menu dismisses it. Then pipe: **`#t:… | back`** (etc.). A trailing incomplete **`tab:`** is stripped when the pipe runs.
...
**タブチップ（`tab:`）:** **`tab:`** で開いているタブ候補（ファビコン＋タイトル）。**`tab:{語句}`** はタイトル絞込、**`tab::{語句}`** は URL 絞込（行に URL 表示）。選択で **`#t:<id>`** チップに置換し、続けて **`tab:`** を付けて再選択可能。Esc または候補先頭での ↑ でメニューを閉じる。その後 **`#t:… | back`** などでパイプ。末尾の未完成 **`tab:`** はパイプ実行時に無視される。
