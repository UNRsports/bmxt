...
All **command-line parsing, the built-in registry, `help` / `man` text, and URL-line handling** live in **`wasm/bmxt-core` (Rust)**. The Service Worker calls **`dispatchFull`** (WASM); the result is either terminal **`lines`** or JSON **`effects`**. Effects that need **`chrome.*`** are executed only in TypeScript (`lib/features/dispatch/apply-effects.ts`). Tab completion candidates come from **`completionCandidatesJson()`** in the same WASM module.

### 日本語

**コマンド行の解釈・組み込みレジストリ・`help` / `man` 本文・URL 行**はすべて **`wasm/bmxt-core`（Rust）** に置いています。Service Worker は **`dispatchFull`**（WASM）の結果を受け取り、**`lines`** ならそのまま表示、**`effects`** なら `lib/features/dispatch/apply-effects.ts` で Chrome API を実行します。Tab 補完候補は WASM の **`completionCandidatesJson`** です。

- **`wasm/bmxt-core`** — `registry`, `dispatch`, `tabs_man`; `serde_json` で Lines / Effects を返す  
...
- **`lib/features/dispatch/`** — Effect 型と Chrome 実行（ブラウザ API の唯一の実装箇所）  
...
1. Add metadata and dispatch logic in **`wasm/bmxt-core`** (`registry::COMMANDS`, `dispatch::handle_command`, new **`Effect`** variant if Chrome is needed).  
2. If the command uses the browser, implement the effect in **`lib/features/dispatch/apply-effects.ts`**.  
3. Extend **`lib/features/dispatch/effect-types.ts`** if you add a new effect shape.  
4. Run **`npm run build:wasm`**, then verify `help`, `man`, and Tab completion.

#### 日本語

1. **`wasm/bmxt-core`** にレジストリ行と `handle_command` の分岐を追加（Chrome が要るなら新しい **`Effect`**）。  
2. ブラウザ操作は **`lib/features/dispatch/apply-effects.ts`** に実装。  
3. 新しい JSON 形なら **`effect-types.ts`** も更新。  
...
- `wasm/bmxt-core/` — Rust コア（レジストリ・dispatch・`help`/`man` 本文）
- `lib/features/wasm-core/` — WASM 初期化・`runDispatch`・補完候補
- `lib/features/dispatch/` — Chrome 向け Effect の型と実行
