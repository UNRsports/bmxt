...
- [Command-line token model (first / second commands)](#command-line-token-model)
  ...
  - [Tab Picker (`picker tabs -list` / `picker tabs -list -u`)](#tabs-tab-picker)
...
| `picker tabs -list [-u]` | Open tab picker column; supports search, multi-select marker `#`, and bulk modes |
...
| `picker dom -list …` | Open a read-only DOM picker column (same picker chrome as `picker search -list …`); mode **`--normal`** or **`--with`**; flavor **`--html`** or **`--react`** |
...
| `picker search -list …` | Open a search picker column with in-picker progress, detail view (**→**), and **`[history]`** open-target tree. **`--all`** (default when you run **`search -list `** with no scope token) searches history, bookmarks, open http(s) tab text, and saved snapshots in parallel |
...
| `picker setting -list` | Open the **settings picker** column (UI locale, appearance, **storage** internal/external, **snapshot storage** bundled/Obsidian vault, export/import zip); changes apply only after **`> save setting`** in the picker |
...
| `picker session -list` | Inline session picker on the prompt (↑↓ · **Enter** / **1–9** switches immediately by index) |
...
- **`picker dom -list …`** + **Enter** opens the DOM picker column (UI path). Bare **`picker dom -list …`** may show the **`--normal` / `--with` / `--html` / `--react` option menu** before the column opens; see **[How columns open](#picker-ui)**.
- **`dom -list [--normal|--with] [--html|--react] [--tag] [<pattern>]`** (with or (plain; use **`picker <list>`** for the column)) resolve the **active tab of the last-focused normal browser window**, inject a read-only helper via `chrome.scripting`, and produce DOM rows. **`chrome://`**, the Chrome Web Store, `chrome-extension://`, etc. are rejected with error lines.
...
| **`picker setting -list`** + **Enter** | Opens the **setting** picker column with a draft copy of current settings. |
...
| **`picker session -list`** + **Enter** | Opens an **inline floating candidate menu** on the prompt (not a side column). Labels `*n  displayName`. **↑↓** · **Enter** / **1–9** switch **immediately**. |
...
**How columns open (UI-first — `picker <list>` or picker-only flows)**

| Input | Behavior |
|-------|----------|
| `picker tabs -list` / `picker tabs -list -u` | Opens the tab picker column |
...
| `picker search -list …` only + **Enter** | Restores **`picker search -list … `** or shows scope menu (see **[`search`](#search-command)**) |
| `picker search -list …` + **Enter** | Runs search in the **search** picker column |
| `picker dom -list …` only + **Enter** | Shows `--normal` / `--with` / `--html` / `--react` option menu when mode/flavor unset |
| `picker dom -list … [--normal|--with] [--html|--react] …` + **Enter** | Fetches DOM output, opens the dom column |
| `picker setting -list` + **Enter** | Opens the settings picker column (see **[`setting`](#setting)**) |
| `translate -on` + **Enter** | Enables translation assist (prompt stays focused) |

**Plain `-list` (terminal log, no column)** — default for **`tabs -list`**, **`dom -list`**, **`search -list`**, **`session -list`**, **`setting -list`** (plain; use **`picker <list>`** for the column). Output uses schema **`bmxt-list/1`**; see **[`-list` output registry](#list-commands-registry)**.

**Settings picker column (`picker setting -list`)**
...
| Key / gesture | search / dom lines | Tab picker (`picker tabs -list`) |
|---------------|------------------|---------------------------|
...
- `picker tabs -list` / `picker tabs -list -u`: open tab picker column (`-u` includes URL rows).
...
### Tab Picker (`picker tabs -list` / `picker tabs -list -u`)
...
| Command | Plain path | `picker <list>` path |
|---------|------------|-----------------|
...
|----------|---------------|------------------|---------------|
| `search-list` | session id | cancel-previous | `bmxt-shell.tsx` (`picker search -list …`) |
| `dom-list` | session id | cancel-previous | `bmxt-shell.tsx` (`picker dom -list …`) |
...
- **Plain `-list` producer:** Add **`lib/features/<feature>/*-list-command.ts`**, register the matcher in **`list-commands/registry.ts`**, and ensure **`plain-list`** / **`picker <list>`** behavior stay consistent (see **`-list` output** above).
...
5. Update **`manifest/bmxt-codegen.json`**, **`bmxt-core/cmd/<module>.ts`**, register **`picker <list>`** kind routing if needed, i18n, and this README / **`_context/map_command.csv`**.
...
- [コマンド一覧](#command-list-ja)
  ...
  - [タブピッカー（`picker tabs -list` / `picker tabs -list -u`）](#tabs-tab-picker-ja)
...
| `picker tabs -list [-u]` | タブピッカー列を開き、検索・複数選択 `#`・バルクモードに対応 |
...
| `picker dom -list …` | DOM 読み取り専用ピッカー列（`picker search -list …` と同系 UI）。mode **`--normal`**／**`--with`**、flavor **`--html`**／**`--react`** |
...
| `picker search -list …` | 検索ピッカー列。走査進捗はピッカー内表示。**→** で詳細一覧または **`[history]`** 開き先ツリー |
...
| `picker setting -list` | **設定ピッカー**列（UI 言語・外観・保存先・snapshot 保存先・zip 入出力）。**`> save setting`** で確定 |
...
| `picker session -list` | プロンプト上のインライン候補（↑↓ · **Enter** / **1–9** で即時切り替え） |
...
**UI ローカルコマンド**（**`RUN_CMD` より前**の **`bmxt-shell.tsx`**）— 例: **`* -list picker <list>`**、**`* -exit -list`**、**`session -list`** / **`setting -list`**（プレーン）、**`session -switch`**、**`translate -on`**、**`nav -enter`** — は React state を直接更新。**プレーン**の **`tabs -list`**、**`dom -list`**、**`search -list`** は **`RUN_CMD`** と **`-list` レジストリ**経由（**[`-list` 出力レジストリ](#list-commands-registry-ja)** 参照）。
...
| **`picker setting -list`** + **Enter** | 現在設定の **draft** を持った **setting** ピッカー列を開く。 |
...
| **`picker session -list`** + **Enter** | プロンプト上の **インライン浮動候補**（横並び列ではない）。表示は `*n  表示名`。**↑↓** · **Enter** / **1–9** で **即時切り替え**。 |
...
**列の開き方（UI 優先 — `picker <list>` またはピッカー専用フロー）**

| 入力 | 動作 |
|------|------|
| `picker tabs -list` / `picker tabs -list -u` | タブピッカー列を開く |
| `group new`（タブ ID なし） | **group-new** variant のタブピッカー列 |
| `search -list` のみ（末尾スペースなし）+ **Enter** | **`search -list `** に復帰（continuation） |
| `picker search -list …` + **Enter** | **search** ピッカー列で検索実行 |
| `picker dom -list …` のみ + **Enter** | mode／flavor 未確定時は `--normal` / `--with` / `--html` / `--react` のオプションメニュー |
| `picker dom -list … [--normal|--with] [--html|--react] …` + **Enter** | DOM 取得後、dom 列を開く |
| `picker setting -list` + **Enter** | 設定ピッカー列を開く（**[`setting`](#setting-ja)** 参照） |
| `translate -on` + **Enter** | 翻訳アシストを有効化（プロンプトにフォーカス維持） |

**プレーン `-list`（ターミナルログ・列なし）** — **`tabs -list`** / **`dom -list`** / **`search -list`** / **`session -list`** / **`setting -list`** が既定。対話列は **`picker <list>`**。出力スキーマは **`bmxt-list/1`**（**[`-list` 出力レジストリ](#list-commands-registry-ja)** 参照）。

**設定ピッカー列（`picker setting -list`）**
...
| キー / 操作 | search / dom 行一覧 | タブピッカー（`picker tabs -list`） |
|-------------|-------------------|------------------------------|
...
- **`picker dom -list …`** + **Enter** で DOM ピッカー列を開く（UI 経路）。裸の **`picker dom -list …`** では mode／flavor 未確定時にオプションメニューを表示することがある（**[列の開き方](#picker-ui-ja)**）。
...
- **`search -list` のみ**（末尾スペースなし）+ **Enter** → **`search -list `** に復帰（continuation）。**`search -list `** + **Enter** → 横断検索 **`--all`** を **プレーン**出力（**`RUN_CMD`** → **`search_list`**）。ピッカー列は **`picker search -list …`**。
- **`search -list [--all|--history|--bookmark|--page|--snapshot] [<pattern>]`**（プレーン）は **`search.hit`** 行を **`-list` レジストリ**で出力。**`picker search -list …`** は search ピッカー列を開く。キャッシュ・live 読み取り・snapshot の詳細は英語節 **[`search`](#search-command)** と同様。
...
- **`picker tabs -list` / `picker tabs -list -u`**：タブピッカー列を開く（`-u` で URL 行付き）。
...
### タブピッカー（`picker tabs -list` / `picker tabs -list -u`）
...
|------------|--------------|------------|----------|
| `search-list` | セッション id | cancel-previous | `bmxt-shell.tsx`（`picker search -list …`） |
| `dom-list` | セッション id | cancel-previous | `bmxt-shell.tsx`（`picker dom -list …`） |
...
- **プレーン `-list` producer:** **`*-list-command.ts`** を追加し **`list-commands/registry.ts`** に matcher を登録。**`plain-list`** / **`picker <list>`** と整合させる。
...
5. **`manifest/bmxt-codegen.json`**、**`bmxt-core/cmd/<module>.ts`**、必要なら **`picker <list>`** の kind ルーティング、i18n、本 README / **`_context/map_command.csv`** を更新。
