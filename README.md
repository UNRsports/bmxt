...
**Tab chips (`tab:`):** Type **`tab:`** to open open-tab candidates (favicon + title). Pick inserts a **`#t:<id>`** chip and **closes** the menu. **Enter** on a chip-only line activates that tab (last chip if several) and focuses its window. Type a **Space** before Enter to reopen candidates and pick more tabs for a pipe. **`tab:{needle}`** (or plain text after Space) filters by title; **`tab::{needle}`** filters by URL. Then pipe: **`#t:… | back`** / **`reload`** / etc. A trailing incomplete **`tab:`** is stripped when the pipe runs.
...
**タブチップ（`tab:`）:** **`tab:`** で開いているタブ候補（ファビコン＋タイトル）。選択で **`#t:<id>`** チップに置換し、**候補は閉じる**。チップのみの行で **Enter** するとそのタブをアクティブ（複数なら最後のチップ）しウィンドウもフォーカス。追加選択は Enter 前に **半角スペース** で候補再表示。タイトル絞込は **`tab:{語句}`** またはスペース後の語句、URL は **`tab::{語句}`**。その後 **`#t:… | back`** / **`reload`** などでパイプ。末尾の未完成 **`tab:`** はパイプ実行時に無視される。
