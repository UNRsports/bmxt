import { loadUiSettingsDirectoryHandle } from "../setting/settings-handle-db"
import { loadUiSettingsStorageConfig } from "../setting/settings-storage-config"
import {
  SNAPSHOTS_BUNDLE_DIR,
  SNAPSHOTS_INDEX_JSON_NAME
} from "./snapshot-bundle-layout"
...
export async function resolveExternalSnapshotsDir(
  bundle: FileSystemDirectoryHandle,
  create: boolean
): Promise<FileSystemDirectoryHandle> {
  return bundle.getDirectoryHandle(SNAPSHOTS_BUNDLE_DIR, { create })
}
...
export async function loadExternalSnapshotsIndex(
...
export async function saveExternalSnapshotsIndex(
  snapshotsDir: FileSystemDirectoryHandle,
...
export async function writeSnapshotToExternalDir(
  bundle: FileSystemDirectoryHandle,
  fileName: string,
  markdown: string,
  meta: SnapshotEntryMeta
): Promise<string> {
  const allowed = await verifyReadWritePermission(bundle)
  if (!allowed) {
    throw new Error("permission denied")
  }
  const snapshotsDir = await resolveExternalSnapshotsDir(bundle, true)
  await writeFileText(snapshotsDir, fileName, markdown)
  const index = await loadExternalSnapshotsIndex(snapshotsDir)
  const withoutDup = index.entries.filter((e) => e.fileName !== fileName)
  const entries = [meta, ...withoutDup]
  await saveExternalSnapshotsIndex(snapshotsDir, { version: 1, entries })
  return `${SNAPSHOTS_BUNDLE_DIR}/${fileName}`
}

export async function listExternalSnapshotDocuments(): Promise<
...
> {
  ...
  const bundle = await loadUiSettingsDirectoryHandle()
  if (!bundle) {
    return null
  }
  const allowed = await verifyReadPermission(bundle)
  if (!allowed) {
    return null
  }
  let snapshotsDir: FileSystemDirectoryHandle
  try {
    snapshotsDir = await resolveExternalSnapshotsDir(bundle, false)
  } catch {
    return []
  }
  const index = await loadExternalSnapshotsIndex(snapshotsDir)
  ...
  for (const meta of index.entries) {
    ...
    out.push({
      meta,
      markdown,
      path: `${SNAPSHOTS_BUNDLE_DIR}/${meta.fileName}`
    })
  }
  return out
}

export async function getExternalBundleHandleForSnapshots(): Promise<FileSystemDirectoryHandle | null> {
  const config = await loadUiSettingsStorageConfig()
  if (config.mode !== "external") {
    return null
  }
  return loadUiSettingsDirectoryHandle()
}

export async function writeAllSnapshotsToExternalBundle(
  bundle: FileSystemDirectoryHandle,
...
export function externalSnapshotPath(fileName: string): string {
  return `${SNAPSHOTS_BUNDLE_DIR}/${fileName}`
}
