/**
 * EN: `nav -reload` tab block tokens (`#t:<id>`) — parse, atomic delete, completion zone.
 */
...
const NAV_RELOAD_LEAD_RE = /^\s*nav\s+-reload\s+/i
 ...
 * Returns null when the browser default character move should run (e.g. still in `nav -reload `).
 */
...
/**
 * EN: Completion zone after `nav -reload ` for picking open tabs.
