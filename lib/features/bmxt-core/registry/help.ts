...
export function buildHelpLines(): string[] {
  ...
  lines.push(
    ...
    "",
    "grep (in-memory search; nothing persisted by these commands):",
    "  grep -list [--none|--history|--bookmark|--page] <pattern> — UI float form (--none = all scopes).",
    "  grep --none <pattern>   — all scopes (same defaults as -list without scope token).",
    "  grep --history <pattern>  — titles/URLs in recent history.",
    "  grep --bookmark <pattern>  — bookmark titles/URLs.",
    "  grep --page <pattern>      — innerText lines in non-discarded http(s) tabs.",
    "",
